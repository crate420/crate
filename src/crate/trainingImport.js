...
function defaultExportPath() {
  return path.join(config.rootDir, "data/training-export.json");
}

function readTrainingExport(exportPath = defaultExportPath()) {
  if (!fs.existsSync(exportPath)) {
    const error = new Error(`Training export not found: ${exportPath}`);
    error.statusCode = 404;
    error.code = "training_export_not_found";
    throw error;
  }
  ...
  for (const key of ["artist_genres", "lastfm_artist_tags", "track_overrides"]) {
    if (!Array.isArray(parsed[key])) {
      const error = new Error(`Training export is missing array: ${key}`);
      error.statusCode = 400;
      error.code = "invalid_training_export";
      throw error;
    }
  }
...
function importTrainingData(options = {}) {
  const exportPath = options.exportPath || defaultExportPath();
  const exportData = readTrainingExport(exportPath);
